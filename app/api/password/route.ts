...
const base = require('airtable').base('appi0Asy4sUqesAXz');
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (name === 'Request Form') {
      ...
      const record = await base('Password').find(process.env.AIRTABLE_PASSWORD_RECORD_ID);
      ...
      const { data: emailData, error: emailError } = await base('Visitors').create([
        {
          "fields": {
